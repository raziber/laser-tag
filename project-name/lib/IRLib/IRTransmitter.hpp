...
public:
    ...
    void transmitToAllPorts(uint32_t address, uint32_t command) const;
    void transmitToSinglePort(uint32_t address, uint32_t command, uint32_t portId) const;
private:
    std::unique_ptr<IREncoder> encoder_;
    std::unique_ptr<IRProtocolSettings> protocolSettings_;
    std::vector<int> ports_;  // A list of ports (RMT channels or GPIO pins)
    int memBlockNum_;
    int clkDiv_;

    void configurePort(int port, int memBlockNum, int clkDiv);
